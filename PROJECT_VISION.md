...
- A **private broker vault** per user — API keys / login secrets for each supported exchange (never shared across accounts)
...
- **Per-user isolation** — strategy vault, broker vault, nodes, and runtime scoped to the account
...
- **No full artifact registry product** — strategy vault is storage + metadata, not devpi/commit
- **No shared platform broker keys** (target) — each user brings their own credentials
...
|-------|--------|------------------|
| **Now (dev/test)** | **Bybit testnet** | Temporary: shared server `.env`. **Target:** per-user broker vault (API key + secret) |
| Most exchanges we add | e.g. Bybit live, others we choose | **Private broker vault** — user stores API keys / secrets; deploy injects them into the trading node only |
| **IBKR** | Interactive Brokers | **Private broker vault** holds IB username + password (+ paper/live). Conductor starts a **dockerized IB Gateway** (same approach as Nautilus `DockerizedIBGateway` / `ghcr.io/gnzsnz/ib-gateway`) beside the trading node; the worker talks to that gateway over the Docker network — not a shared host TWS |

### Broker credentials vault (target)

Separate from the **strategy vault**. Each user has a **private broker vault**:

- One credential set per supported adapter (or named profiles: e.g. `bybit-testnet`, `bybit-live`)
- Secrets encrypted at rest; never returned in full by the API after save; never shared across users
- On deploy, the API resolves the chosen broker profile → injects opaque `broker.config` into the Conductor command (same complete-deploy rule as today)

**API-key brokers (Bybit and most others):** credentials alone are enough — trading node connects to the exchange REST/WS APIs directly.

**IBKR (special):** IB requires a running TWS / IB Gateway process. Conductor (or the trading-node bootstrap path) launches the **Nautilus-supported IB Gateway Docker image** for that user/node, using vaulted IB login + trading mode. The trading node then connects to that container (`ibg_host` / port on `conductor-net`). Gateway lifecycle is tied to the node (stop/delete tears it down with the worker, or as an explicit companion service we manage).

**Today:** dashboard still injects shared Bybit keys from server `.env`. Broker vault + IB Gateway containers are **planned**.
...
Auth, stamp `user_id` on commands, durable node records + quota, strategy vault resolve, **broker vault** resolve, enqueue Conductor commands, serve node list, on-demand TCP snapshot. Later: consume observe stream, WebSockets, audit log.

**Today:** FastAPI — register/login/JWT, strategy vault CRUD/share, dashboard lifecycle (deploy/run/halt/stop/restart/delete/status/snapshot). Broker vault not built yet (shared `.env` Bybit).

### 3. Node orchestrator (Conductor Node)

Container/process lifecycle. Validates deploy envelope, allocates a unique control port, writes bootstrap, spawns trading node (and for IBKR, the companion **IB Gateway** container). Proxies strategy control over TCP.
...
Durable users, strategy vault, broker vault (planned), trading node records, (later) audit.

**Today:** shared PostgreSQL via `conductor-core` — `users`, `strategies`, `strategy_access`, `trading_nodes`. Conductor’s live registry remains in-memory. No per-user broker secrets table yet.
...
### Target (broker vault + IB Gateway + observe)

```
User → API: save broker profile in private broker vault (Bybit keys, or IB user/pass)
     → API: pick strategy vault entry + broker profile + config
     → DB: node record + desired state
     → Conductor: if IBKR → start dockerized IB Gateway (Nautilus image) for this node
     → Conductor: spawn trading node with broker.config from vault (never from browser)
...
| Broker adapters (Bybit, IBKR, …) | Per-user broker vault; IBKR → dockerized Gateway |
| `ImportableStrategyConfig`, `Controller` | In-node control handlers |
| Live data, order execution | UI/API to start/stop/monitor |
| `DockerizedIBGateway` (IB container pattern) | Conductor-managed Gateway sidecar per IBKR node |
...
| Bybit testnet broker | Working — default for dashboard deploy (shared `.env` keys) |
| IBKR adapter | Code exists; **target:** per-user vault + dockerized IB Gateway (Nautilus image) |
| API auth (register, login, JWT) | Working |
| API dashboard lifecycle + snapshot | Working — JWT + username as `user_id` |
| Shared PostgreSQL (`conductor-core`) | Working — users, strategy vault, trading_nodes |
| Strategy vault | Working — DB globals / owned / share; zip upload later |
| **Broker vault (per-user API keys)** | Planned — private credentials per supported exchange |
...
| 6 | Broker vault + IB Gateway containers + observe + production UI | **Next** |

---

## Success in one paragraph

Conductor succeeds when a user can put Nautilus strategies in a vault, store their own broker credentials privately, deploy a worker from the UI within their quota (with IBKR spinning up a dockerized Gateway like Nautilus does), start and stop it remotely, inspect live state when they need it, and trust the dashboard because ownership and lifecycle are durable — without SSH, without a custom strategy framework, and without brokers or features Nautilus does not already support.
