"""Discover strategy metadata from a local or cloud artifact."""
...
def _normalize_slug_from_path(source_path: str) -> str:
    name = source_path.strip().replace("\\", "/").split("/")[-1]
    if name.endswith(".py"):
        name = name[:-3]
    if not name or name.startswith("_") or not name.isidentifier():
        raise ValueError(
            f"Invalid strategy path '{source_path}'. "
            "File stem must be a Python identifier (e.g. running_ping.py).",
        )
    return name


def discover_strategy_from_file(filename: str) -> DiscoveredStrategy:
    """Shorthand: register from repo ``strategies/<filename>``."""
    return discover_strategy_from_location(ArtifactLocation.local_strategies(filename))


def discover_strategy_from_location(location: ArtifactLocation) -> DiscoveredStrategy:
    """
    Materialize artifact (local / S3 / GCS) and extract Strategy + StrategyConfig.
    """
    slug = _normalize_slug_from_path(location.source_path)
    try:
    ...
    # Live import only works for in-repo strategies.* today
...
def _find_classes_via_ast(source: str, slug: str) -> tuple[str, str]:
    try:
        ...
        raise ValueError(f"Cannot parse strategy source for '{slug}': {exc}") from exc
    ...
    if not strategy_classes:
        raise ValueError(f"No Strategy subclass found in '{slug}'")
